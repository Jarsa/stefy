...
class AccountMove(models.Model):
    ...
    def action_electronic_invoice_sent(self):
        ...
        report_template_id = self.env.ref(
                invoice.journal_id.template_id.xml_id)._render_qweb_pdf(self.id)
    ...
    @api.depends("move_type", "iDest", "fiscal_position_id")
    ...
    def _is_invoice_sale(self):
        self.ensure_one()
        return self.move_type in ["out_invoice"]
    ...
    def _prepare_gTot_values(self):
        ...
        for count, line in enumerate(self.line_ids.filtered(lambda l: l.account_id.user_type_id.type == "receivable"),
            ...
            if self.iDoc in ["4", "6"] and self.move_type == "out_refund":
    ...
    def _prepare_dGen_values(self):
        ...
        if self.iDoc == "4" and self.move_type == "out_refund" and all([self.has_reconciled_entries, reversal_entry_id ]):
