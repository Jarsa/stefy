...
class AccountInvoiceSend(models.TransientModel):
    ...
    def _print_document(self):
        ...
        if all([move.factura_result_id.is_success, move.factura_result_id.apcon_id, move.cufe] for move in self.invoice_ids):
        ...
        if self.is_electronic_invoice:
            for move in self.invoice_ids:
                if not all([move.factura_result_id.is_success, move.factura_result_id.apcon_id, move.cufe]):
                    ...
                    raise ValidationError("Electronic Invoice Is not yet Success on this move: %s" % move.name)
                else:
    ...
    def _get_mail_template(self):
        ...
        return (
            ...
            if all(move.move_type == 'out_refund' for move in self.invoice_ids)
